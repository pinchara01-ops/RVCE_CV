...
real network calls - the live tier is exercised via a mocked client.
"""
import json
...
@pytest.mark.parametrize("failure_reason,setup", [
    ...
    ("malformed_json", lambda monkeypatch: monkeypatch.setattr(
        "query_retrieval.gemini_client.get_client", lambda: _FakeClient("not json at all"),
    )),
    ("missing_match_field", lambda monkeypatch: monkeypatch.setattr(
        "query_retrieval.gemini_client.get_client",
